...
impl TransactionService {
    ...
    pub async fn create(
        ...
        let transaction = self.transaction_repository.create(request).await?;

        Ok(transaction.into())
    }

    pub async fn list(&self, user_id: Uuid) -> Result<Vec<TransactionResponse>, AppError> {
        let portfolios = self.portfolio_repository.list_by_user(user_id).await?;

        let mut all_transactions = Vec::new();

        for portfolio in portfolios {
            let mut transactions = self.transaction_repository.list(portfolio.id).await?;
    ...
    pub async fn get(&self, id: Uuid, user_id: Uuid) -> Result<TransactionResponse, AppError> {
    ...
    pub async fn update(
        ...
        let transaction = self.transaction_repository.update(id, request).await?;
